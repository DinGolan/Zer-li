...
public class EchoServer extends AbstractServer 
{
  ...
  public void handleMessageFromClient(Object msg, ConnectionToClient client)
  {
	   ...
	   if(((Message)msg).getOption().compareTo("Get all orders numbers for this customer can cancel") == 0) //get all the orders that connected to specific customer and can cancel
	    ...
	    {
	    	((Message)msg).setMsg(Update_Tables_For_Testing(msg,conn));  
	    	this.sendToAllClients(msg); 
	    }
	    
	    if(((Message)msg).getOption().compareTo("Test - Give - Me The Size Of Order_Table") == 0) 	    	
	    {
	    	((Message)msg).setMsg(Return_Size_Of_Order_Table(msg,conn));  
	    	this.sendToAllClients(msg);
	    }
	    
	    if(((Message)msg).getOption().compareTo("Test - Bring Me The Order That I Add To Table - For Test") == 0) 	    	
	    {
	    	((Message)msg).setMsg(Bring_All_Order_For_Test(msg,conn));  
	    	this.sendToAllClients(msg);
	    }
...
protected void Delete_After_Test(Object msg , Connection conn)
	  ...
	  try {
		  	...
		  	for(int i = requested_Order_Num.size() - 1 ; i >= 0  ; i--)
		  	{
			  	...
			  	while(rs.next())
			  	{
			  		...
			  		Payment = Account.PaymentMethod.valueOf(rs.getString("paymentMethod"));
					o.setPaymentMethod(Payment);
					Status = Order.orderStatus.valueOf(rs.getString("orderStatus"));
					o.setoStatus(Status);
					Refund = rs.getDouble("orderRefund");
					o.setRefund(Refund);
					Order_For_Test.add(o);
			  	}
		  	}
   ...
   * @param conn - Connection to DB .
   */
  protected void Test_Cancel_Order(Object msg , Connection conn)
  {
	  ...
	  {	
		  ...
		  stmt = conn.createStatement();
		  String Update_Cancel_Order_From_Order_Table = "UPDATE " + Scheme_Name + ".order SET orderStatus =" + "'" + Order_To_Cancel.getoStatus()  + "'" + "WHERE orderID = " + "'" + Order_To_Cancel.getOrderID() + "'" + ";" ;
		  stmt.executeUpdate(Update_Cancel_Order_From_Order_Table);
		  String Update_Refund_Order_From_Order_Table = "UPDATE " + Scheme_Name + ".order SET orderRefund =" + "'" + Order_To_Cancel.getRefund()  + "'" + "WHERE orderID = " + "'" + Order_To_Cancel.getOrderID() + "'" + ";" ;
		  stmt.executeUpdate(Update_Refund_Order_From_Order_Table);
		  
		  String Get_Customer_From_Account_Table = "SELECT * FROM " + Scheme_Name + ".account WHERE " +  "AccountUserId = " + "'" + Order_To_Cancel.getCustomerID() + "'" + ";" ;
		  ResultSet rs = stmt.executeQuery(Get_Customer_From_Account_Table);
		  
		  while(rs.next())
		  {
			  Store_ID = rs.getInt("AccountStoreId");
			  if(Store_ID == Order_To_Cancel.getStoreID())
			  {
   ...
   * @param conn - Connection to DB .
   */
  protected Integer Return_Size_Of_Order_Table(Object msg , Connection conn)
  {
		 ...
		 /* Count The Number Of Rows In The Table */
		  ...
		  {
			  stmt = conn.createStatement();
			  String Take_The_Num_Of_Rows_In_Order_Table = "SELECT * FROM " + Scheme_Name + ".order " + ";";
  ...
  protected Integer Update_Tables_For_Testing(Object msg , Connection conn)
  {
	      ...
	      ArrayList<Integer> Temp_Number_Of_Rows = (ArrayList<Integer>)(((Message)msg).getMsg());
	 
	      Number_Of_Rows_In_Order_Table = Temp_Number_Of_Rows.get(0);
		  double Balance = 0 ;
		  Vector <ArrayList<String>> Update_Tabels_For_Test = new Vector <ArrayList<String>>();
		  ArrayList<String> SQL_Queries_Order = new ArrayList <String>();
		  ArrayList <String> SQL_Queries_Product_In_Order = new ArrayList <String>();
		  ArrayList <String> Customer_ID = new ArrayList<String>();
		  ArrayList <String> Price_Of_Order = new ArrayList<String>();
		  ArrayList <String> Stores_ID = new ArrayList<String>();	
		  ...
		  String Formatted_Date_String_For_Test = localDate.format(Formatter);
		  
	      String Formatted_Date_String_For_Test_One = localDate.format(Formatter);
	      String Formatted_Date_String_For_Test_Two = localDate.format(Formatter);
	      String Formatted_Date_String_For_Test_Three = localDate.format(Formatter);
	      String Formatted_Date_String_For_Test_Four = localDate.format(Formatter);
	      
	      if((nowTime.isAfter(LocalTime.parse("22:00"))) && (nowTime.isBefore(LocalTime.parse("00:00"))))
	      {
	    	  if(nowTime.plusHours(2).isAfter(LocalTime.parse("00:00")))
	    	  {
	    		  localDate = LocalDate.now().plusDays(1);
	    		  Formatter = DateTimeFormatter.ofPattern("yyyy-MM-dd");
	    		  Formatted_Date_String_For_Test_One = localDate.format(Formatter);
	    	  }
	      }
	      
	      if((nowTime.isAfter(LocalTime.parse("23:30"))) && (nowTime.isBefore(LocalTime.parse("00:00"))))
	      {
	    	  if(nowTime.plusMinutes(30).isAfter(LocalTime.parse("00:00")))
	    	  {
	    		  localDate = LocalDate.now().plusDays(1);
	    		  Formatter = DateTimeFormatter.ofPattern("yyyy-MM-dd");
	    		  Formatted_Date_String_For_Test_Two = localDate.format(Formatter);
	    	  }
	      }
	      
	      if((nowTime.isAfter(LocalTime.parse("17:00"))) && (nowTime.isBefore(LocalTime.parse("00:00"))) )
	      {
	    	  if(nowTime.plusHours(7).isAfter(LocalTime.parse("00:00")))
	    	  {
	    		  localDate = LocalDate.now().plusDays(1);
	    		  Formatter = DateTimeFormatter.ofPattern("yyyy-MM-dd");
	    		  Formatted_Date_String_For_Test_Three = localDate.format(Formatter);
	    	  }
	      }
	      
	      if((nowTime.isAfter(LocalTime.parse("19:00"))) && (nowTime.isBefore(LocalTime.parse("00:00"))) )
	      {
	      
	    	  if(nowTime.plusHours(5).isAfter(LocalTime.parse("00:00")))
	    	  {
	    		  localDate = LocalDate.now().plusDays(1);
	    		  Formatter = DateTimeFormatter.ofPattern("yyyy-MM-dd");
	    		  Formatted_Date_String_For_Test_Four = localDate.format(Formatter);
	    	  }
	      }
		  ...
		  String Second_New_Order = "INSERT INTO " + Scheme_Name + ".`order`(`orderID`, `customerID`, `orderSupplyOption`, `orderTotalPrice`, `orderRequiredSupplyDate`, `orderRequiredSupplyTime`, `orderRecipientAddress`, `orderRecipientName`, `orderRecipientPhoneNumber`, `orderPostcard`, `orderDate`, `StoreID`, `paymentMethod`, `orderStatus`, `orderRefund`) VALUES " + "(" + "'" + (++Number_Of_Rows_In_Order_Table) + "'" + ", '308155127', 'DELIVERY', '42', " + "'" + Formatted_Date_String_For_Test_Two + "'" + ", " + "'" + nowTime.plusMinutes(30) + "'" + ", 'Tel - Aviv', 'Alon Golany', '0545254564', 'Thank You', " + "'" + Formatted_Date_String_For_Test + "'" + ", '4', 'CASH', 'APPROVED', '0');" ;
		  ...
		  /* More Than Three Hour's From The Final Time To Make Delivery */			
		  String Fourth_New_Order = "INSERT INTO " + Scheme_Name + ".`order` (`orderID`, `customerID`, `orderSupplyOption`, `orderTotalPrice`, `orderRequiredSupplyDate`, `orderRequiredSupplyTime`, `orderRecipientAddress`, `orderRecipientName`, `orderRecipientPhoneNumber`, `orderPostcard`, `orderDate`, `StoreID`, `paymentMethod`, `orderStatus`, `orderRefund`) VALUES " + "(" + "'" + (++Number_Of_Rows_In_Order_Table) + "'" + ", '308155160', 'DELIVERY', '60', " + "'" + Formatted_Date_String_For_Test_Four + "'" + ", " + "'" + nowTime.plusHours(5) + "'" + ", 'Jerusalem', 'Itay Golany', '0545255555', 'Thank You', " + "'" + Formatted_Date_String_For_Test + "'" + ", '2', 'CASH', 'APPROVED', '0');" ;
		  ...
		  Update_Tabels_For_Test.add(SQL_Queries_Order);
		  Update_Tabels_For_Test.add(SQL_Queries_Product_In_Order);
		  Update_Tabels_For_Test.add(Customer_ID);
		  Update_Tabels_For_Test.add(Price_Of_Order);
		  Update_Tabels_For_Test.add(Stores_ID);
	  ...
	  return Number_Of_Rows_In_Order_Table;
  }
